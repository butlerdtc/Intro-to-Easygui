...
while True:
    ...
    while True:
        ...
        if sequence1 == "Stick":
            if results[0:5].count == 5:
                easygui.msgbox(f"You rolled: {player1_roll}\n\nYahtzee!",
                               "Winner")
                player1_score += 50
            elif results[0:4].count or results[1:5] == 4:
                easygui.msgbox(f"You rolled: {player1_roll}\n\nFour of a "
                               f"kind!", "Winner")
                player1_score += 30
            elif (results[0:3].count or results[1:4].count or
                  results[2:5].count) == 3:
                easygui.msgbox(
        ...
        if sequence2 == "Stick":
            if results[0:5].count == 5:
                ...
                easygui.msgbox(f"You rolled: {player2_roll}\n\nFour of a "
                               f"kind!", "Winner")
                player2_score += 30
            elif (results[0:3].count or results[1:4].count or results[2:5].
                    count == 3):
                easygui.msgbox(
                    f"You rolled: {player2_roll}\n\nThree of a kind!",
                    "Winner")
